...
export default  class TypeRow extends Component {
    ...
    render() {
        ...
        var modalTrue = <Modal type="confirm" eventRow={event}>你确定奖励已经收到吗？</Modal>;
        ...
        var ButtonTrue = <ModalTrigger modal={modalTrue}  onConfirm={this.onConfirm}><Button >确认奖励</Button></ModalTrigger>
        return (
            <tr>
                <td>{event.id}</td>
                <td>{event.name}</td>
                ...
                <td>{event.description}</td>
                <td>{ButtonDetailed} {ButtonTrue}</td>
            </tr>
        );
    }}
